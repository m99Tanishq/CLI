...
# Rzork CLI IntelliJ Plugin Build Script
# This script builds the IntelliJ plugin from source

set -e

echo "🚀 Building Rzork CLI IntelliJ Plugin..."
...
if [ ! -f "gradlew" ]; then
    ...
    # Try to use gradle if available
    if command -v gradle &> /dev/null; then
        gradle wrapper
    else
        echo "⚠️  Gradle not found. Creating wrapper manually..."
...
if [ $? -eq 0 ]; then
    ...
    if [ -n "$PLUGIN_FILE" ]; then
        ...
        echo "⚠️  Warning: Plugin file not found in build/distributions/"
    fi
else
    echo "❌ Build failed!"
    exit 1
